...
const sections: Section[] = [
  ...
  {
    title: "Projects",
    links: [{ name: "Tend", href: "https://usetend.org", description: "Nurture your relationships", icon: TendIcon, external: true }],
  },
  {
    title: "Socials & More",
    links: [
...
const sectionsWithOffsets = sections.reduce<Array<{ title: string; links: LinkItem[]; offset: number }>>((acc, section) => {
  ...
  return (
    <div className="mx-auto max-w-[540px] px-5 py-10">
      ...
      <div className="animate-fade-in mb-10 flex flex-col items-center text-center">
        ...
        >
          <Image src="/images/profile.jpg" alt={siteConfig.name} fill className="object-cover" priority sizes="112px" />
        </div>

        <h1 className="font-display text-[clamp(2rem,8vw,3rem)] leading-none tracking-tight">{siteConfig.name.toUpperCase()}</h1>

        <p className="font-mono-label text-muted mt-4 max-w-[34ch]">{siteConfig.linksPageDescription}</p>
      </div>
            ...
            <div className="space-y-4">
              {section.links.map((link, i) => {
                ...
                return (
                  ...
                  >
                    {/* Left color stripe */}
                    <div aria-hidden="true" className="w-3 flex-shrink-0" style={{ background: stripe, borderRight: "3px solid var(--border)" }} />

                    {/* Content */}
                    <div className="flex flex-1 items-center gap-4 px-4 py-4 sm:px-5">
                      <Icon className="h-6 w-6 flex-shrink-0" />
                      <div className="flex-1 text-left">
                        <div className="font-display text-base tracking-tight sm:text-lg">{link.name.toUpperCase()}</div>
                        {link.description && <div className="font-mono-label text-muted mt-0.5">{link.description}</div>}
                      </div>
                      {link.external ? (
